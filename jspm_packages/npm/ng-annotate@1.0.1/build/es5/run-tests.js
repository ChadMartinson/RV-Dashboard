...
(function(process) {
  ...
  var ngAnnotateAdfPlugin = require("./optionals/angular-dashboard-framework");
  ...
  function testSourcemap(original, got, sourcemap) {
    ...
    function testMapping(needle) {
      ...
      var originalPosition = smc.originalPositionFor({
        line: gotPosition.line,
        column: gotPosition.col
      });
      ...
      if (originalPosition.line !== expectedPosition.line || originalPosition.column !== expectedPosition.col) {
        process.stderr.write(fmt("Sourcemap mapping error for {0}. Expected: ({1},{2}) => ({3},{4}). Got: ({5},{6}) => ({3},{4}).", needle, expectedPosition.line, expectedPosition.col, gotPosition.line, gotPosition.col, originalPosition.line, originalPosition.column));
        process.exit(-1);
      }
    }
  ...
  function run(ngAnnotate) {
    ...
    test(annotated, ngAnnotate(annotated, {
      add: true,
      remove: true
    }).src, "with_annotations.js");
    ...
    var compiledSourcemaps = coffee.compile(originalSourcemaps, {
      sourceFiles: ["sourcemaps.coffee"],
      generatedFile: "sourcemaps.js",
      sourceMap: true
    });
    var annotatedSourcemaps = ngAnnotate(compiledSourcemaps.js, {
      remove: true,
      add: true,
      map: {sourceRoot: "/source/root/dir"}
    });
    ...
    var inlinedCompiledSourcemaps = compiledSourcemaps.js + os.EOL + convertSourceMap.fromJSON(compiledSourcemaps.v3SourceMap).toComment();
    var combinedSourcemaps = ngAnnotate(inlinedCompiledSourcemaps, {
      remove: true,
      add: true,
      map: {
        inline: true,
        inFile: "sourcemaps.js",
        sourceRoot: "/source/root/dir"
      }
    });
    ...
    var ngminAnnotated = ngAnnotate(ngminOriginal, {
      add: true,
      regexp: "^myMod"
    }).src;
    ...
    test(ngminOriginal, ngAnnotate(ngminAnnotated, {
      remove: true,
      regexp: "^myMod"
    }).src, "ngmin_original.js");
    console.log("testing optionals/angular-dashboard-framework.js (adding annotations)");
    var adf = slurp("tests/optionals/angular-dashboard-framework.js");
    var adfAnnotated = ngAnnotate(adf, {
      add: true,
      plugin: [ngAnnotateAdfPlugin]
    }).src;
    ...
    test(adf, ngAnnotate(adfAnnotated, {
      remove: true,
      plugin: [ngAnnotateAdfPlugin]
    }).src, "optionals/angular-dashboard-framework.js");
    ...
    if (fs.existsSync("tests/angular.js")) {
      ...
      time("  ng1 processed in {0} ms", function() {
        ngAnnotate(ng1, {add: true});
      });
      time("  ng1 processed with sourcemaps in {0} ms", function() {
        ngAnnotate(ng1, {
          add: true,
          map: true
        });
