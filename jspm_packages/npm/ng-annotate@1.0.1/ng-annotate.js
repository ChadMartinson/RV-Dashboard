...
(function(process) {
  ...
  const optimist = require("optimist").usage("ng-annotate v" + version + "\n\nUsage: ng-annotate OPTIONS <file>\n\n" + "provide - instead of <file> to read from stdin\n" + "use -a and -r together to remove and add (rebuild) annotations in one go").options("a", {
    alias: "add",
    boolean: true,
  ...
  }).options("o", {describe: "write output to <file>. output is written to stdout by default"}).options("sourcemap", {
    boolean: true,
    describe: "generate an inline sourcemap"
  }).options("sourceroot", {describe: "set the sourceRoot property of the generated sourcemap"}).options("single_quotes", {
    boolean: true,
    describe: "use single quotes (') instead of double quotes (\")"
  }).options("regexp", {describe: "detect short form myMod.controller(...) iff myMod matches regexp"}).options("rename", {
    describe: "rename declarations and annotated references\n" + "oldname1 newname1 oldname2 newname2 ...",
    default: ""
  }).options("plugin", {describe: "use plugin with path (experimental)"}).options("enable", {describe: "enable optional with name"}).options("list", {
