...
(function(process) {
  ...
  const ngAnnotateAdfPlugin = require("./optionals/angular-dashboard-framework");
  ...
  function run(ngAnnotate) {
    ...
    const compiledSourcemaps = coffee.compile(originalSourcemaps, {
      sourceFiles: ["sourcemaps.coffee"],
      generatedFile: "sourcemaps.js",
      sourceMap: true
    });
    const annotatedSourcemaps = ngAnnotate(compiledSourcemaps.js, {
      remove: true,
      add: true,
    ...
    const inlinedCompiledSourcemaps = compiledSourcemaps.js + os.EOL + convertSourceMap.fromJSON(compiledSourcemaps.v3SourceMap).toComment();
    const combinedSourcemaps = ngAnnotate(inlinedCompiledSourcemaps, {
      remove: true,
      add: true,
      map: {
        inline: true,
    ...
    const ngminAnnotated = ngAnnotate(ngminOriginal, {
      add: true,
      regexp: "^myMod"
    }).src;
    ...
    const adfAnnotated = ngAnnotate(adf, {
      add: true,
