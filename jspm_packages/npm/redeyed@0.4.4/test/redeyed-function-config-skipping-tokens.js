...
test('given i skip 2 more tokens after each semicolon', function(t) {
  var calls = 0,
      opts = {Punctuator: {';': function identity(s, info) {
            calls++;
            var skipToken = info.tokens[info.tokenIndex + 2];
            return skipToken ? {
              replacement: s,
              skipPastToken: skipToken
            } : s;
          }}};
  ;
  [{
...
test('replace log', function(t) {
  var kinds = ['info', 'warn', 'error'],
      opts = {Identifier: {console: function replaceLog(s, info) {
            var code = info.code,
                idx = info.tokenIndex,
                tokens = info.tokens,
      ...
      origCode = ['console.info("info ", 1);', 'console.warn("warn ", 3);', 'console.error("error ", new Error("oh my!"));'].join('\n'),
