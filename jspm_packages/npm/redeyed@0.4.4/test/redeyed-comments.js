...
test('\nstring config, Line comments', function(t) {
  ...
  t.test('\n# ' + inspect(opts), function(t) {
    t.assertSurrounds('// a comment', opts, '*// a comment&');
    t.assertSurrounds('// comment then new line\nif (a == 1) return', opts, '*// comment then new line&\nif (a == 1) return');
    t.assertSurrounds('var n = new Test();// some comment after\n//more comment\nvar s = 3;', opts, 'var n = new Test();*// some comment after&\n*//more comment&\nvar s = 3;');
    t.end();
  });
});
