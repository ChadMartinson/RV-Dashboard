...
require("../_base").ext.profile = ext = function(conf) {
  ...
  if (!stack.split('\n').slice(3).some(function(line) {
    if ((line.indexOf('/memoizee/') === -1) && (line.indexOf('/es5-ext/') === -1) && (line.indexOf('/next/lib/fs/_memoize-watcher') === -1) && (line.indexOf(' (native)') === -1)) {
      id = line.replace(/\n/g, "\\n").trim();
      return true;
    }
  })) {
...
exports.log = function() {
  ...
  forEach(stats, function(data, name) {
    ...
    return (this[b].initial + this[b].cached) - (this[a].initial + this[a].cached);
  });
