...
(function(process) {
  ...
  var packages = require("./packages.json!systemjs-json");
  var nopt = require("nopt");
  var options = nopt({'force': Boolean}, {'f': '--force'});
  var env = {
  ...
  function ensurePackage(dir) {
    ...
    return promise.fail(function() {
      return Q.nfcall(mkdirp, dir).then(cmd.bind(null, 'git', ['init'], {cwd: dir})).then(function() {
        return Q.nfcall(fs.writeFile, path.join(dir, '.master'), 'based on master');
      }).then(cmd.bind(null, 'git', ['add', '-A'], {cwd: dir})).then(function() {
  ...
  function checkRelease(dir, release) {
    ...
    return cmd('git', ['branch', '--list'], {cwd: dir}).spread(function(stdout) {
  ...
  function createRelease(dir, release, files) {
    ...
    return cmd('git', ['checkout', 'master', '-f'], {cwd: dir}).then(function() {
      return cmd('git', ['branch', '-D', branch], {cwd: dir}).fail(function() {});
    }).then(cmd.bind(null, 'git', ['checkout', '-b', branch, 'master'], {cwd: dir})).then(function() {
      var promise;
      var promises = [];
      mout.object.forOwn(files, function(contents, name) {
        ...
        promise = Q.nfcall(mkdirp, path.dirname(name)).then(function() {
      ...
      return cmd('git', ['tag', '-f', release], {cwd: dir}).then(cmd.bind(null, 'git', ['checkout', 'master', '-f'], {cwd: dir})).then(cmd.bind(null, 'git', ['branch', '-D', branch], {cwd: dir}));
    });
  }
  var promises = [];
  mout.object.forOwn(packages, function(pkg, name) {
    ...
    mout.object.forOwn(pkg, function(files, release) {
      promise = promise.then(checkRelease.bind(null, dir, release)).then(function(exists) {
        if (exists) {
