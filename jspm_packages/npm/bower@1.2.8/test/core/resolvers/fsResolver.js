...
(function(process) {
  ...
  describe('FsResolver', function() {
    ...
    before(function(next) {
      logger = new Logger();
      cmd('git', ['checkout', '0.2.1'], {cwd: testPackage}).then(next.bind(next, null), next);
    });
    ...
    describe('.hasNew', function() {
      it('should resolve always to true (for now..)', function(next) {
        ...
        fs.writeFileSync(path.join(tempSource, '.bower.json'), JSON.stringify({name: 'test'}));
        resolver.hasNew(tempSource).then(function(hasNew) {
    ...
    describe('.resolve', function() {
      function assertMain(dir, singleFile) {
        return Q.nfcall(fs.readFile, path.join(dir, '.bower.json')).then(function(contents) {
      ...
      it('should copy the source file, renaming it to index', function(next) {
        ...
        resolver.resolve().then(function(dir) {
          ...
          return assertMain(dir, 'index.md').then(next.bind(next, null));
        }).done();
      });
      it('should rename to index if source is a folder with just one file in it', function(next) {
        ...
        copy.copyFile(path.join(testPackage, 'foo'), path.join(tempSource, 'foo')).then(resolver.resolve.bind(resolver)).then(function(dir) {
          expect(fs.existsSync(path.join(dir, 'index'))).to.be(true);
          expect(fs.existsSync(path.join(dir, 'foo'))).to.be(false);
      ...
      it('should not rename to index if source is a folder with just bower.json/component.json file in it', function(next) {
        ...
        mkdirp.sync(tempSource);
        resolver = create(tempSource);
        copy.copyFile(path.join(testPackage, 'bower.json'), path.join(tempSource, 'bower.json')).then(resolver.resolve.bind(resolver)).then(function(dir) {
          expect(fs.existsSync(path.join(dir, 'bower.json'))).to.be(true);
          rimraf.sync(tempSource);
          mkdirp.sync(tempSource);
          resolver = create(tempSource);
        }).then(copy.copyFile.bind(copy, path.join(testPackage, 'bower.json'), path.join(tempSource, 'component.json'))).then(function() {
          return resolver.resolve();
        }).then(function(dir) {
      ...
      it('should copy the source directory permissions', function(next) {
        ...
        resolver = create(tempSource);
        copy.copyDir(testPackage, tempSource).then(function() {
          fs.chmodSync(tempSource, 0777);
          mode0777 = fs.statSync(tempSource).mode;
        }).then(resolver.resolve.bind(resolver)).then(function(dir) {
      ...
      it('should copy the source file permissions', function(next) {
        ...
        resolver = create(tempSource);
        copy.copyFile(path.join(testPackage, 'foo'), tempSource).then(function() {
          fs.chmodSync(tempSource, 0777);
          mode0777 = fs.statSync(tempSource).mode;
        }).then(resolver.resolve.bind(resolver)).then(function(dir) {
