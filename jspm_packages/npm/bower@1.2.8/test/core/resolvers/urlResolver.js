...
describe('UrlResolver', function() {
  ...
  describe('.hasNew', function() {
    ...
    it('should work with redirects', function(next) {
      ...
      var resolver;
      nock(redirectingUrl).head('/foo.js').reply(302, '', {location: redirectingToUrl + '/foo.js'});
  ...
  describe('.resolve', function() {
    ...
    it('should extract if response content-disposition filename is an archive', function(next) {
      var resolver;
      nock('http://bower.io').get('/package-zip').replyWithFile(200, path.resolve(__dirname, '../../assets/package-zip.zip'), {'Content-Disposition': 'attachment; filename="package-zip.zip"'});
      resolver = create('http://bower.io/package-zip');
      resolver.resolve().then(function(dir) {
        expect(fs.existsSync(path.join(dir, 'foo.js'))).to.be(true);
        expect(fs.existsSync(path.join(dir, 'bar.js'))).to.be(true);
    ...
    it('should work with redirects', function(next) {
      ...
      var redirectingToUrl = 'http://bower.io';
      var resolver;
      nock(redirectingUrl).get('/foo.js').reply(302, '', {location: redirectingToUrl + '/foo.js'});
    ...
    describe('content-disposition validation', function() {
      function performTest(header, extraction) {
        var resolver;
        nock('http://bower.io').get('/package-zip').replyWithFile(200, path.resolve(__dirname, '../../assets/package-zip.zip'), {'Content-Disposition': header});
        resolver = create('http://bower.io/package-zip');
