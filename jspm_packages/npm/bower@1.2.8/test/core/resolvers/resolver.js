...
(function(process) {
  ...
  describe('Resolver', function() {
    ...
    describe('.hasNew', function() {
      ...
      beforeEach(function() {
        fs.writeFileSync(path.join(tempDir, '.bower.json'), JSON.stringify({name: 'test'}));
      });
      after(function(next) {
        rimraf(tempDir, next);
      });
      it('should throw an error if already working (resolving)', function(next) {
        ...
        resolver.hasNew(tempDir).then(function() {
          succeeded = true;
        }, function(err) {
      ...
      it('should not read the package meta if already passed', function(next) {
        ...
        resolver.hasNew(tempDir, {name: 'foo'}).then(function() {
          expect(meta).to.be.an('object');
    ...
    describe('.resolve', function() {
      ...
      it('should call all the functions necessary to resolve by the correct order', function(next) {
        ...
        resolver.resolve().then(function() {
          expect(resolver.getStack()).to.eql(['before _createTempDir', 'after _createTempDir', 'before _readJson', 'after _readJson', 'before _applyPkgMeta', 'after _applyPkgMeta', 'before _savePkgMeta', 'after _savePkgMeta']);
          next();
        }).done();
      });
    ...
    describe('._createTempDir', function() {
      ...
      it('should remove the folder after execution', function(next) {
        ...
        rimraf(defaultConfig.tmp, function(err) {
          if (err)
            return next(err);
          cmd('node', ['test/assets/test-temp-dir/test.js'], {cwd: path.resolve(__dirname, '../../..')}).then(function() {
      ...
      it('should remove the folder on an uncaught exception', function(next) {
        rimraf(defaultConfig.tmp, function(err) {
          if (err)
            return next(err);
          cmd('node', ['test/assets/test-temp-dir/test-exception.js'], {cwd: path.resolve(__dirname, '../../..')}).then(function() {
    ...
    describe('._applyPkgMeta', function() {
      ...
      it('should remove files that match the ignore patterns', function(next) {
        ...
        mkdirp.sync(tempDir);
        cmd('git', ['checkout', '0.2.2'], {cwd: testPackage}).then(function() {
    ...
    describe('._savePkgMeta', function() {
      ...
      it('should save the package meta to the package meta file (.bower.json)', function(next) {
        var resolver = create('foo');
        resolver._tempDir = tempDir;
        resolver._savePkgMeta({name: 'bar'}).then(function(retMeta) {
