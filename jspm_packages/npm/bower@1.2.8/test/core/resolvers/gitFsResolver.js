  ...
  describe('GitFsResolver', function() {
    ...
    describe('.resolve', function() {
      ...
      it('should leave the original repository untouched', function(next) {
        cmd('git', ['checkout', 'master'], {cwd: testPackage}).then(function() {
          var resolver = create({
            source: testPackage,
            target: 'some-branch'
          });
          ...
          return cmd('git', ['branch', '--color=never'], {cwd: testPackage}).spread(function(stdout) {
            expect(stdout).to.contain('* master');
          });
        }).then(function() {
          return cmd('git', ['status', '--porcelain'], {cwd: testPackage}).spread(function(stdout) {
            stdout = stdout.trim();
    ...
    describe('#refs', function() {
      ...
      it('should resolve to the references of the local repository', function(next) {
        GitFsResolver.refs(testPackage).then(function(refs) {
          refs = refs.slice(1, 8);
      ...
      it('should cache the results', function(next) {
        GitFsResolver.refs(testPackage).then(function() {
