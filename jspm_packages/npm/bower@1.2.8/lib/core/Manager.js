...
(function(process) {
  ...
  Manager.prototype.resolve = function() {
    ...
    return this._deferred.promise.fin(function() {
      this._working = false;
    }.bind(this));
  };
  Manager.prototype.install = function() {
    ...
    return Q.nfcall(mkdirp, componentsDir).then(function() {
      ...
      mout.object.forOwn(that._dissected, function(decEndpoint, name) {
        ...
        promise = Q.nfcall(rimraf, dst).then(copy.copyDir.bind(copy, decEndpoint.canonicalDir, dst)).then(function() {
          var metaFile = path.join(dst, '.bower.json');
          decEndpoint.canonicalDir = dst;
          return Q.nfcall(fs.readFile, metaFile).then(function(contents) {
  ...
  Manager.prototype._fetch = function(decEndpoint) {
    ...
    this._nrFetching++;
    return decEndpoint.promise = this._repository.fetch(decEndpoint).spread(this._onFetchSuccess.bind(this, decEndpoint)).fail(this._onFetchError.bind(this, decEndpoint));
  };
  ...
  Manager.prototype._dissect = function() {
    ...
    mout.object.forOwn(this._resolved, function(decEndpoints, name) {
      ...
      promise = promise.then(function() {
        return that._electSuitable(name, semvers, nonSemvers).then(function(suitable) {
    ...
    promise.then(function() {
      ...
      this._dissected = mout.object.filter(suitables, function(decEndpoint, name) {
        ...
        var dst;
        if (installedMeta && installedMeta._target === decEndpoint.target && installedMeta._originalSource === decEndpoint.source && installedMeta._release === decEndpoint.pkgMeta._release) {
          return false;
        }
        dst = path.join(componentsDir, name);
    ...
    }.bind(this)).then(this._deferred.resolve, this._deferred.reject);
  };
    ...
    if (semvers.length && nonSemvers.length) {
      ...
      suitable = mout.array.find(semvers, function(subject) {
        return semvers.every(function(decEndpoint) {
          return subject === decEndpoint || semver.satisfies(subject.pkgMeta.version, decEndpoint.target);
        });
      });
      if (suitable) {
    ...
    if (resolution && !unresolvable) {
      ...
      if (semver.validRange(resolution)) {
        suitable = mout.array.findIndex(picks, function(pick) {
          return pick.pkgMeta.version && semver.satisfies(pick.pkgMeta.version, resolution);
        });
      }
      if (suitable === -1) {
        suitable = mout.array.findIndex(picks, function(pick) {
          return pick.target === resolution || pick.pkgMeta._release === resolution;
        });
      }
      if (suitable === -1) {
  ...
  Manager.prototype._areCompatible = function(candidate, resolved) {
    ...
    if (semver.validRange(candidate.target) && semver.validRange(resolved.target)) {
      ...
      return highestCandidate.version && highestResolved.version && semver.eq(highestCandidate.version, highestResolved.version) && highestCandidate.comparator === highestResolved.comparator && semver.satisfies(resolvedVersion, candidate.target);
    }
    return false;
  };
