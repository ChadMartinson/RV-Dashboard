...
ResolveCache.prototype.retrieve = function(source, target) {
  ...
  return this._getVersions(sourceId).spread(function(versions) {
    ...
    return that._readPkgMeta(canonicalDir).then(function(pkgMeta) {
...
ResolveCache.prototype.store = function(canonicalDir, pkgMeta) {
  ...
  return promise.then(function(pkgMeta) {
    ...
    return Q.nfcall(fs.stat, dir).then(function() {
      ...
      return Q.nfcall(fs.rename, canonicalDir, dir).fail(function(err) {
        if (err.code !== 'EXDEV') {
          throw err;
        }
        return copy.copyDir(canonicalDir, dir).then(function() {
...
ResolveCache.prototype.clear = function() {
  return Q.nfcall(rimraf, this._dir).then(function() {
...
ResolveCache.prototype.list = function() {
  ...
  var dirs = [];
  var that = this;
  return Q.nfcall(fs.readdir, this._dir).then(function(sourceIds) {
    promises = sourceIds.map(function(sourceId) {
      return Q.nfcall(fs.readdir, path.join(that._dir, sourceId)).then(function(versions) {
    ...
    promises = dirs.map(function(dir) {
      return that._readPkgMeta(dir).then(function(pkgMeta) {
        return {
          canonicalDir: dir,
          pkgMeta: pkgMeta
        };
...
ResolveCache.prototype._getVersions = function(sourceId) {
  ...
  return Q.nfcall(fs.readdir, dir).then(function(versions) {
    that._sortVersions(versions);
