...
(function(process) {
  ...
  Project.prototype.install = function(decEndpoints, options) {
    ...
    this._working = true;
    return this._analyse().spread(function(json, tree) {
  ...
  Project.prototype.update = function(names, options) {
    ...
    this._options = options || {};
    this._working = true;
    return this._analyse().spread(function(json, tree, flattened) {
      if (!names) {
        ...
        names.forEach(function(name) {
          if (!flattened[name]) {
            throw createError('Package ' + name + ' is not installed', 'ENOTINS', {name: name});
          }
        });
        that.walkTree(tree, function(node, name) {
          if (node.extraneous && node.linked) {
      ...
      return that._bootstrap(targets, resolved, incompatibles).then(function(installed) {
        return that.saveJson().then(function() {
          return installed;
        });
      });
      ...
      names.forEach(function(name) {
        ...
        promise = promise.then(function() {
          ...
          if (!that._config.interactive) {
            throw createError(message, 'ECONFLICT', {data: data});
          }
      ...
      if (onlyOnce) {
        dependencies = dependencies.filter(function(dependency) {
          return !mout.array.find(onlyOnce, function(stacked) {
            ...
            return dependency.name === stacked.name && dependency.source === stacked.source && dependency.target === stacked.target;
          });
        });
  ...
  Project.prototype.saveJson = function(forceCreate) {
    ...
    return Q.nfcall(fs.writeFile, file, jsonStr).then(function() {
  ...
  Project.prototype.hasJson = function() {
    return this._readJson().then(function(json) {
  ...
  Project.prototype._analyse = function() {
    return Q.all([this._readJson(), this._readInstalled(), this._readLinks()]).spread(function(json, installed, links) {
      var root;
  ...
  Project.prototype._bootstrap = function(targets, resolved, incompatibles) {
    ...
    return this._manager.configure({
      targets: targets,
      resolved: resolved,
    ...
    }.bind(this)).then(this._manager.install.bind(this._manager));
  };
  Project.prototype._readJson = function() {
    ...
    return this._json = readJson(this._config.cwd, {assume: {name: path.basename(this._config.cwd) || 'root'}}).spread(function(json, deprecated, assumed) {
      var jsonStr;
      if (deprecated) {
  ...
  Project.prototype._readLinks = function() {
    ...
    var that = this;
    componentsDir = path.join(this._config.cwd, this._config.directory);
    return Q.nfcall(fs.readdir, componentsDir).then(function(filenames) {
      var promises;
      var decEndpoints = {};
      promises = filenames.map(function(filename) {
        ...
        return validLink(dir).spread(function(valid, err) {
          ...
          return readJson(dir, {assume: {name: name}}).spread(function(json, deprecated) {
            if (deprecated) {
  ...
  Project.prototype._removePackages = function(packages) {
    ...
    mout.object.forOwn(packages, function(dir, name) {
      ...
      if (!dir) {
        promise = Q.resolve();
        that._logger.warn('not-installed', name, {name: name});
      } else {
