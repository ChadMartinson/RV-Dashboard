...
(function(process) {
  ...
  GitFsResolver.prototype._checkout = function() {
    ...
    return this._copy().then(cmd.bind(cmd, 'git', ['checkout', '-f', resolution.tag || resolution.branch || resolution.commit], {cwd: this._tempDir})).then(cmd.bind(cmd, 'git', ['clean', '-f', '-d'], {cwd: this._tempDir}));
  };
  ...
  GitFsResolver.refs = function(source) {
    ...
    value = cmd('git', ['show-ref', '--tags', '--heads'], {cwd: source}).spread(function(stdout) {
      var refs;
