...
UrlResolver.prototype._resolve = function() {
  return this._download().spread(this._parseHeaders.bind(this)).spread(this._extract.bind(this)).then(this._rename.bind(this));
};
...
UrlResolver.prototype._extract = function(file, response) {
  ...
  return extract(file, this._tempDir, {mimeType: mimeType});
};
UrlResolver.prototype._rename = function() {
  return Q.nfcall(fs.readdir, this._tempDir).then(function(files) {
    var file;
    var oldPath;
    var newPath;
...
UrlResolver._cacheHeaders = ['Content-MD5', 'ETag', 'Last-Modified', 'Content-Language', 'Content-Length', 'Content-Type', 'Content-Disposition'];
