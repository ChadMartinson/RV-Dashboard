...
FsResolver.prototype._resolve = function() {
  return this._copy().then(this._extract.bind(this)).then(this._rename.bind(this));
};
FsResolver.prototype._copy = function() {
  ...
  return Q.nfcall(fs.stat, this._source).then(function(stat) {
    ...
    if (stat.isDirectory()) {
      dst = that._tempDir;
      promise = that._readJson(that._source).then(function(json) {
      ...
      promise = copy.copyFile(that._source, dst, copyOpts).then(function() {
        return dst;
      });
    }
