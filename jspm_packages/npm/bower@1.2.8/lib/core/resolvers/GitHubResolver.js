...
function GitHubResolver(decEndpoint, config, logger) {
  ...
  if (!pair) {
    throw createError('Invalid GitHub URL', 'EINVEND', {details: this._source + ' does not seem to be a valid GitHub URL'});
  }
  this._org = pair.org;
...
GitHubResolver.prototype._checkout = function() {
  ...
  return download(tarballUrl, file, {
    ...
    return extract(file, that._tempDir).fail(function(err) {
      ...
      return that._cleanTempDir().then(GitRemoteResolver.prototype._checkout.bind(that));
    });
  }, function(err) {
