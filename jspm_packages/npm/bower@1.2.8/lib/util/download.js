...
function download(url, file, options) {
  ...
  operation.attempt(function() {
    ...
    req = progress(request(url, options), {delay: progressDelay}).on('response', function(res) {
    ...
    writeStream = req.pipe(fs.createWriteStream(file)).on('error', deferred.reject).on('close', function() {
