...
function clean(endpoints, options, config) {
  ...
  Q.all([clearPackages(decEndpoints, config, logger), clearLinks(names, config, logger), !names ? clearCompletion(config, logger) : null]).spread(function(entries) {
    return entries;
    ...
    if (decEndpoints) {
      entries = entries.filter(function(entry) {
        return !!mout.array.find(decEndpoints, function(decEndpoint) {
          ...
          if (decEndpoint.name !== entryPkgMeta.name && decEndpoint.source !== entryPkgMeta.name && decEndpoint.source !== entryPkgMeta._source) {
            return false;
          }
          ...
          return decEndpoint.target === entryPkgMeta._target || decEndpoint.target === entryPkgMeta._release;
        });
      });
    }
    promises = entries.map(function(entry) {
      return repository.eliminate(entry.pkgMeta).then(function() {
        logger.info('deleted', 'Cached package ' + entry.pkgMeta.name + ': ' + entry.canonicalDir, {file: entry.canonicalDir});
      });
    });
...
function clearLinks(names, config, logger) {
  ...
  return promise.then(function(names) {
    ...
    promises = names.map(function(name) {
      ...
      return Q.nfcall(fs.readlink, link).then(function(linkTarget) {
        return Q.nfcall(fs.stat, linkTarget).then(function(stat) {
          if (!stat.isDirectory()) {
    ...
    return Q.all(promises).then(function() {
      ...
      promises = linksToRemove.map(function(link) {
        return Q.nfcall(rimraf, link).then(function() {
          logger.info('deleted', 'Invalid link: ' + link, {file: link});
        });
      });
...
function clearCompletion(config, logger) {
  ...
  return Q.nfcall(fs.stat, dir).then(function() {
    return Q.nfcall(rimraf, dir).then(function() {
      logger.info('deleted', 'Completion cache', {file: dir});
    });
  }, function(error) {
