...
PassThrough.prototype = Object.create(Stream.prototype, {
  constructor: {value: PassThrough},
  write: {value: function(c) {
      this.emit('data', c);
    }},
  end: {value: function(c) {
      if (c)
        this.write(c);
      this.emit('end');
    }},
...
tap.test('replace with ~YARG~', function(t) {
  ...
  str.pipe(ms);
  var expect = ['foo', '~YARG~~YARG~~YARG~~YARG~~YARG~', 'bar', '~YARG~~YARG~~YARG~', 'baz', 'boo', '~YARG~~YARG~', '~YARG~~YARG~~YARG~~YARG~'];
  ms.on('data', function(c) {
    t.equal(c, expect.shift());
  });
  str.write('foo');
